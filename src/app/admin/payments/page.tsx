            ...
            <tbody>
              {loading ? <tr><td className="py-8 px-4 text-muted-foreground" colSpan={7}>Loading payments…</td></tr> : filtered.map((payment) => (
                <tr key={payment.id} className="border-b last:border-0">
                  ...
                  <td className="py-3 px-4"><div className="font-mono text-xs">{payment.orders?.order_number ?? payment.order_id}</div><div className="font-mono text-xs font-semibold tracking-wider">ID: {payment.orders?.payment_reference ?? "—"}</div></td>
