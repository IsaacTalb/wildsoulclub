...
export function ProductDetailSkeleton() {
  ...
  return (
    <div
      className="min-h-screen overflow-x-clip bg-white py-5 md:-mt-[var(--site-header-height)] md:h-svh md:min-h-0 md:overflow-hidden md:px-4 md:pb-4 md:pt-[calc(var(--site-header-height)+1.5rem)] lg:h-auto lg:min-h-screen lg:overflow-y-visible"
      role="status"
    ...
    >
      <div className="container mx-auto max-w-[1300px]">
        <div className="grid grid-cols-1 items-start gap-7 md:grid-cols-[minmax(0,1fr)_minmax(320px,0.58fr)] lg:grid-cols-[minmax(0,3fr)_minmax(320px,2fr)] lg:gap-12">
          <Skeleton
            className={`${skeletonClass} aspect-[4/5] min-h-[480px] w-full rounded-[1.75rem] sm:min-h-[560px] md:aspect-auto md:h-[calc(100svh-var(--site-header-height)-5.5rem)] md:min-h-[580px] md:max-h-[1080px] lg:h-[calc(100svh-var(--site-header-height))] lg:min-h-0 lg:max-h-none lg:rounded-none`}
          />

          <div className="relative z-20 mx-4 w-[calc(100%-2rem)] rounded-[1.75rem] border border-black/10 bg-white/95 p-4 shadow-[0_18px_50px_rgba(15,23,42,0.08)] sm:p-5 md:mx-0 md:w-full md:max-w-[440px] md:justify-self-end md:p-5 lg:sticky lg:top-[calc(var(--site-header-height)+1rem)] lg:max-h-[calc(100svh-var(--site-header-height)-2rem)] lg:max-w-none lg:overflow-y-auto lg:overscroll-contain">
